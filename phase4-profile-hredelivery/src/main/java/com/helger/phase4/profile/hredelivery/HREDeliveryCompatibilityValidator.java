...
{
  public HREDeliveryCompatibilityValidator ()
  {}
                                          ...
                                          @Nonnull @Nonempty final String sFieldPrefix)
  {
    ...
    if (aPModeLegSecurity != null)
    {
      ...
      {
        aErrorList.add (_createError (sFieldPrefix + "Security.X509EncryptionAlgorithm must not be used"));
      }
    ...
    else
    {
      ...
      if (aFrom != null)
      {
        if (aFrom.getPartyIdCount () > 1)
          ...
          if (aFrom.getPartyIdCount () == 1)
          {
            if (!HREDeliveryPMode.DEFAULT_PARTY_TYPE_ID.equals (aFrom.getPartyIdAtIndex (0).getType ()))
            {
              aErrorList.add (_createError ("PartyInfo/From[0]/@type must be '" +
                                            HREDeliveryPMode.DEFAULT_PARTY_TYPE_ID +
                                            "' instead of '" +
      ...
      if (aTo != null)
      {
        if (aTo.getPartyIdCount () > 1)
          ...
          if (aTo.getPartyIdCount () == 1)
          {
            if (!HREDeliveryPMode.DEFAULT_PARTY_TYPE_ID.equals (aTo.getPartyIdAtIndex (0).getType ()))
            {
