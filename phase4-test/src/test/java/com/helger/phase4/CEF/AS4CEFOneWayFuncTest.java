...
public final class AS4CEFOneWayFuncTest extends AbstractCEFTestSetUp
{
   ...
   * (One-Way/Push MEP). SMSH is simulated to send an AS4 User message to the RMSH with parameter
   * MESSAGEPROPERTIES containing: A property with attributes "name" and "type" present. A property
   * with only attribute "name" present.<br>
   * <br>
   * Predicate: <br>
   ...
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile:
   * One-Way/Push MEP. SMSH sends an AS4 message (User Message with payload) to the RMSH.<br>
   * <br>
   * Predicate: <br>
   ...
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). Producer submits a Message with metadata information and XML payload to the
   * SMSH.<br>
   * <br>
   * Predicate: <br>
   ...
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). Producer submits a Message with metadata information and payload to the
   * SMSH.<br>
   * <br>
   * Predicate: <br>
   * In the AS4 message generated by the SMSH, a property element with name "CompressionType" and
   * value set to "application/gzip" is present.
   *
   * @throws Exception
   *         In case of error
   */
   ...
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). Producer submits a message to the SMSH with payload (ex: xml document) and
   * metadata information including a property element with name "MimeType" and value
   * ("application/xml").<br>
   * <br>
   * Predicate: <br>
   * The SMSH generates an AS4 message with the property "MimeType" present and set to the value
   * specified by the producer ("application/xml").
   *
   * @throws Exception
   *         In case of error
   */
   ...
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). The SMSH is simulated to send an AS4 message without property "MimeType"
   * present to the RMSH.<br>
   * <br>
   * Predicate: <br>
   ...
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). Producer submits a message to the SMSH with xml (UTF-16) payload and
   * metadata information including payload character set info.<br>
   * <br>
   * Predicate: <br>
   * The SMSH generates an AS4 message with the property "CharacterSet" present and set to the value
   * "UTF-16".
   *
   * @throws Exception
   *         In case of error
   ...
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). Producer submits a message to the SMSH with xml (UTF-8) payload and
   * metadata information including payload character set info.<br>
   * <br>
   * Predicate: <br>
   * The SMSH generates an AS4 message with the property "CharacterSet" present and set to the value
  ...
  /**
   * Note: Not testable (might become valid after the requirements EBXMLMSG-87 and EBXMLMSG-88 are
   * validated). <= thats what is standing in the document<br>
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). SMSH is simulated to send an AS4 message with property element
   * "CharacterSet" set to value not conform to section 4.3.3 of [XML10] (example: "!utf*"). The
   * SMSH sends the AS4 message to the RMSH.<br>
   * <br>
   * Predicate: <br>
   ...
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). SMSH is simulated to send an AS4 User Message with compressed but damaged
   * payloads. The SMSH sends the AS4 User Message to the RMSH.<br>
   * <br>
   * Predicate: <br>
   * The RMSH sends back a synchronous error response with error code "Code = EBMS:0303, Short
   * Description = DecompressionFailure, Severity = Failure, Category = Communication".
   *
   * @throws Exception
   *         In case of error
   */
   ...
   * Predicate: <br>
   * The RMSH sends back a synchronous error response (with error code "Code = EBMS:0303, Short
   * Description = DecompressionFailure, Severity = Failure, Category = Communication") and the RMSH
   * sends an error notification to the consumer of the message.
   */
  @Test
   ...
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). SMSH sends an AS4 User Message with a compressed payload to the RMSH.<br>
   * <br>
   * Predicate: <br>
   ...
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). SMSH sends an AS4 User Message with a several compressed payloads (XML and
   * non XML) to the RMSH.<br>
   * <br>
   * Predicate: <br>
   ...
   * Prerequisite:<br>
   * eSENS_TA13.<br>
   * The SMSH is simulated to send an AS4 User message with a compressed then signed payload to the
   * RMSH.<br>
   * <br>
   * Predicate: <br>
   ...
   * Prerequisite:<br>
   * eSENS_TA13<br>
   * Simulated SMSH sends a signed AS4 User Message with a signed then compressed payload to the
   * RMSH.<br>
   * <br>
   * Predicate: <br>
   ...
   * Prerequisite:<br>
   * eSENS_TA14.<br>
   * The SMSH is simulated to send a compressed then encrypted AS4 message to the RMSH.<br>
   * <br>
   * Predicate: <br>
   * The RMSH sends back an AS4 non-repudiation receipt.
   *
   ...
   * Prerequisite:<br>
   * eSENS_TA14.<br>
   * Simulated SMSH sends a signed AS4 User Message with an encrypted first, then compressed payload
   * to the RMSH.<br>
   * <br>
   * Predicate: <br>
   * The SMSH receives a WS-Security SOAP Fault.
  ...
  public void testAS4_TA20 () throws Exception
  {
    ...
    XMLTransformerFactory.newTransformer ().transform (xmlSource, outputTarget);
   ...
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). The SMSH sends an AS4 message with a compressed then encrypted and signed
  ...
  /**
   * Note: This test assertion is only valid in case TLS is handled by the AS4 message handler.
   * AS4_TA22 - AS4_TA26 all TLS tests
   */
  @Test
   ...
   * <br>
   * Predicate: <br>
   * In the message sender|receiver elements reference the MSHs and not the (producer,consumer).
   *
   * @throws Exception
   *         In case of error
   */
   ...
   * Prerequisite:<br>
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). Producer submits two payloads, first being an SBDH document, second being
   * an actual payload (non-XML payload). SMSH sends an AS4 User Message to the RMSH.<br>
   * <br>
   * Predicate: <br>
   * Message has two additional MIME parts. The first mime part is the SBDH document and the second
   ...
   * SMSH and RMSH are configured to exchange AS4 messages according to the e-SENS profile
   * (One-Way/Push MEP). SMSH is simulated to send an AS4 User message to the RMSH with parameter
   * PARTPROPERTIES containing: A property with attributes "name" and "type" present. A property
   * with only attribute "name" present. <br>
   * <br>
   * Predicate: <br>
   * The RMSH returns a non-repudiation receipt within a HTTP response with status code 2XX.
