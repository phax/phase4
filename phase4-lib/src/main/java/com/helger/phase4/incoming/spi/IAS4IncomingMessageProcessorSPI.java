 ...
 * Name before v3: <code>com.helger.phase4.servlet.IAS4ServletMessageProcessorSPI</code>
 *
...
public interface IAS4IncomingMessageProcessorSPI
{
   ...
   * @param aPayload
   *        Extracted, decrypted and verified payload node (e.g. SBDH). May be <code>null</code>.
   *        May also be <code>null</code> if a MIME message comes in - in that case the SOAP body
   *        MUST be empty and the main payload can be found in aIncomingAttachments[0].
   * @param aIncomingAttachments
   *        Extracted, decrypted and verified attachments. May be <code>null</code> or empty if no
   *        attachments are present.
   * @param aIncomingState
   *        The current message state. Can be used to determine all other things potentially
   *        necessary for processing the incoming message. Never <code>null</code>.
   ...
   * Attachment and Payload are not needed since they are allowed, but should not be added to a
   * SignalMessage Because the will be ignored in the MSH - Processing.
   *
   * @param aIncomingMessageMetadata
   ...
   *        maybe empty for positive responses. Since v4.3.0.
   */
  void processAS4ResponseMessage (@NonNull IAS4IncomingMessageMetadata aIncomingMessageMetadata,
                                  ...
                                  @NonNull AS4ErrorList aEbmsErrorMessages);
}
