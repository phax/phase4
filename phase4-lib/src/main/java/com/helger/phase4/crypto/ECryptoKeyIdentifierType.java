 ...
 * Deprecated constants have been taken over deprecated.
 *
...
public enum ECryptoKeyIdentifierType implements IHasID <String>
{
   ...
   * the receiver. The X509 profile recommends to use {@link #ISSUER_SERIAL} instead of sending the
   * whole certificate. <br>
   * Please refer to WS Security specification X509 1.1 profile, chapter 3.3.2 and to WS Security
   * SOAP Message security 1.1 specification, chapter 7.2 <br>
   * Note: only local references to BinarySecurityToken are supported
   */
  BST_DIRECT_REFERENCE ("bst", WSConstants.BST_DIRECT_REFERENCE),
  /**
   * Send the issuer name and the serial number of a certificate to the receiver. <br>
   * In contrast to {@link #BST_DIRECT_REFERENCE} only the issuer name and the serial number of the
   * signing certificate are sent to the receiver. This reduces the amount of data being sent. The
   * encryption method uses the public key associated with this certificate to encrypt the symmetric
   * key used to encrypt data. <br>
   ...
   * <code>KeyIdentifier</code> token and sent to the receiver. Thus the complete certificate data
   * is transfered to receiver. The X509 profile recommends to use {@link #ISSUER_SERIAL} instead of
   * sending the whole certificate. <br>
   * Please refer to WS Security SOAP Message security 1.1 specification, chapter 7.3. Note that
   * this is a NON-STANDARD method. The standard way to refer to an X.509 Certificate via a
   * KeyIdentifier is to use {@link #SKI_KEY_IDENTIFIER}
   */
  X509_KEY_IDENTIFIER ("x509-key-id", WSConstants.X509_KEY_IDENTIFIER),
  /**
   * Send a <code>SubjectKeyIdentifier</code> to identify the signing certificate. <br>
  ...
  /**
   * Send a <code>SubjectKeyIdentifier</code> as ds:X509Data/ds:KeyValue element to refer to the
   * base64 encoded plain value of a X509 V.3 SubjectKeyIdentifier extension
   */
  X509_SKI_KEY_IDENTIFIER ("x509-ski-key-id", WSConstants.X509_SKI),
  /**
   * <code>UT_SIGNING</code> is used internally only to set a specific Signature behavior. The
   * signing token is constructed from values in the UsernameToken according to WS-Trust
   * specification.
   */
  ...
  /**
   * <code>CUSTOM_SYMM_SIGNING</code> is used internally only to set a specific Signature behavior.
   ...
   * EncryptedKeySHA1. This identifier uses the SHA-1 digest of a security token to identify the
   * security token. Please refer to chapter 7.3 of the OASIS WSS 1.1 specification.
   */
   ...
   * behavior. The signing key, reference id and value type are set externally.
   */
  CUSTOM_SYMM_SIGNING_DIRECT ("custom-symm-signing-default", WSConstants.CUSTOM_SYMM_SIGNING_DIRECT),
  /**
   * <code>CUSTOM_KEY_IDENTIFIER</code> is used to set a KeyIdentifier to a particular ID The
   * reference id and value type are set externally.
   */
   ...
   * identifier. Constant is useful in case of symmetric holder of key, where token service can
   * determine target service public key to encrypt shared secret.
   */
  ENDPOINT_KEY_IDENTIFIER ("endpoint-key-id", WSConstants.ENDPOINT_KEY_IDENTIFIER),
  /**
   * Sets the <code>org.apache.wss4j.dom.message.WSSecSignature.build(Crypto)</code> or the
   * <code>org.apache.wss4j.dom.message.WSSecEncrypt.build(Crypto, SecretKey)</code> method to send
   * the issuer name and the serial number of a certificate to the receiver.<br>
   * In contrast to {@link #BST_DIRECT_REFERENCE} only the issuer name and the serial number of the
   * signing certificate are sent to the receiver. This reduces the amount of data being sent. The
   * encryption method uses the public key associated with this certificate to encrypt the symmetric
   * key used to encrypt data. The issuer name format will use a quote delimited Rfc 2253 format if
   * necessary which is recognized by the Microsoft's WCF stack. It also places a space before each
   * subsequent RDN also required for WCF interoperability. In addition, this format is know to be
   * correctly interpreted by Java.<br>
   * Please refer to WS Security specification X509 1.1 profile, chapter 3.3.3
   */
